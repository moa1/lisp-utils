    ...
    (flet ((keyword-name (arg)
      ...
      (loop for arg in (walker:llist-optional llist) do
	   (cond
		...
		(set-result! (walker:parameter-suppliedp arg) (walker:make-nil arg))))
	     (t
	      (set-result! (walker:parameter-var arg) (pop arguments))
	      (when (walker:parameter-suppliedp arg)
		(set-result! (walker:parameter-suppliedp arg) (walker:make-object t arg))))))
  ...
  (labels ((recurse! (ast)
    ...
    (etypecase ast
	 ...
	 (loop for arg in init-args do
		...
		(setf (walker:parameter-init arg) (walker:make-nil arg))))

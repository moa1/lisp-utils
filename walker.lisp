    ...
    (cond
	 ...
	 (cond
	   ...
	   ((eq head 'multiple-value-call)
	    (assert (and (consp rest) (listp (cdr rest))) () "Cannot parse MULTIPLE-VALUE-CALL-form ~S" form)
	    (let* ((function-form (car rest))
		   (body (cdr rest))
		   (current (make-instance 'multiple-value-call-form :parent parent))
		   (parsed-function (reparse function-form current))
		   (parsed-body (loop for form in body collect (reparse form current))))
	      (setf (form-function current) parsed-function (form-body current) parsed-body)
	      current))
	   ((find head '(go macrolet multiple-value-prog1 progv symbol-macrolet tagbody unwind-protect))
